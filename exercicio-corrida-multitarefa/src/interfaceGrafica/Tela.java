...
public class Tela {
    ...
    private void criarCompetidores() {
        ...
        competidores.add(new Corredor("Michel", 15));
        competidores.add(new Corredor("Bárbara", 8));
        competidores.add(new Corredor("Lucas", 16));
        competidores.add(new Corredor("Yasmim", 10));
    }
    ...
    private void iniciarCorrida() {
        ...
        if (distanciaDaCorrida > 0) {
            ...
            // Percorre a lista dos competidores, prepara eles para uma nova corrida e acrescenta a thread
            ...
            // Enquanto há uma corrida em andamento, a corrida é atualizada e a thread é suspensa por 100 milissegundos
            Thread atualizacaoAutomatica = (
                new Thread() {
                    @Override
                    public void run() {
                        try {
                            while (corridaEstaAcontecendo()) {
                                atualizarCorrida();
                                Thread.sleep(100);
                            }
                            atualizarCorrida();
            ...
            atualizacaoAutomatica.start();
        }
    }
        
    /*
     * Retorna se há algum competidor correndo
     */
    private boolean corridaEstaAcontecendo (){
        for (Competidor competidor : competidores){
